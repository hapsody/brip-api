...
const nearbySearchInnerAsyncFn = async (param: NearBySearchParams) => {
...
export const nearbySearch = asyncWrapper(
  async (
    req: Express.IBTypedReqBody<NearBySearchParams>,
    res: Express.IBTypedResponse<IBResFormat>,
  ) => {
    const response = await nearbySearchInnerAsyncFn(req.body);
    res.json({
      ...ibDefs.SUCCESS,
  ...
  async (
    ...
    const data = await (async () => {
      ...
      const options = {
        ...
        headers: {
          'X-RapidAPI-Key':
            'ed5143522dmsh08a8f16dd35fd7ap1433aajsn5ba513b80047',
          'X-RapidAPI-Host': 'booking-com.p.rapidapi.com',
        },
      };
      const response = await axios.request(options);
