/// AdPlaceDraft + AdPlace 체계로 개편하면서 Draft를 관리자가 승인하면 바로 TourPlace가 생성되도록 approveAdPlaceDraft api를 추가함
/// 이에 따라 approved 상태로만 변경 후 DailyBatch를 통해 TourPlace를 생성하는 본 스크립트는 주석처리하여 코드만 보관한다.
...
const registTPFromAdPlace = async (): Promise<void> => {
