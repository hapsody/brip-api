...
let gTotalCount: number = 999999;
let gLastItemIdx: number = 0;
dotenv.config();
...
const reqToTour4 = async <T>({
  ...
  // const { items } = data;
  // return items.item;
  gTotalCount = data.totalCount;
  return data;
};

async function injectTPDataToDB(params: {
  pageNo: number;
  numOfRows: number;
}): Promise<void> {
            ...
            case 'A0302': /// 육상레포츠
              switch (v.cat3) {
                ...
                case 'A03021100': /// 승마
                  return ['landActivity>horseRiding'];
                case 'A03021200': /// 스키/스노보드
                  return ['mountainActivity>ski', 'mountainActivity>snowBoard'];
                case 'A03021800': /// 암벽등반
                  return ['mountainActivity>rockClimbing'];
                case 'A03022100': /// ATV
                  return ['landActivity>ATV'];
  ...
  const lastItemIdx = gLastItemIdx;
  // eslint-disable-next-line no-restricted-syntax
...
async function main(): Promise<void> {
  try {
    ...
    const maxItemCount = 100000; /// 존재한다면 maxItemCount만큼 항목을 읽어들일때까지 계속 반복함
    const numOfRows = 600; /// TourAPI4.0 데이터에서 페이지당 읽어올 아이템 수
    const arr = new Array(Math.ceil(maxItemCount / numOfRows)).fill(0);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    for await (const [index, dummyValue] of arr.entries()) {
      /// 만약에 Tour4.0 api에서 보유한 데이터의 totalCount가 현재까지 스크립트가 읽어들인 항목수보다 많다면(아직 읽을게 남았으니까) 계속반복
      if (gTotalCount <= index * numOfRows) {
        break;
      }
      await injectTPDataToDB({
        pageNo: index,
        numOfRows,
      });
    }
